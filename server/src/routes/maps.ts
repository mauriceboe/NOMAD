...
// Category mapping for nearby place types
...
function resolveNearbyCategoryId(type: string): number | null {
  ...
  // Best-effort match against existing categories by name keywords
...
router.post('/nearby', authenticate, async (req: Request, res: Response) => {
  ...
  if (!NEARBY_TYPE_GROUPS[type]) {
    return res.status(400).json({ error: `Invalid type. Must be one of: ${Object.keys(NEARBY_TYPE_GROUPS).join(', ')}` });
  }
  ...
  if (!apiKey) {
    // Fallback: Nominatim + Overpass for OSM nearby search
    try {
      ...
      if (!osmFilter) return res.status(400).json({ error: 'Unsupported type for OpenStreetMap search' });
      const query = `[out:json][timeout:10];(node[${osmFilter}](around:${searchRadius},${parsedLat},${parsedLng});way[${osmFilter}](around:${searchRadius},${parsedLat},${parsedLng}););out center tags ${adminMaxResults};`;
      const overpassRes = await fetch('https://overpass-api.de/api/interpreter', {
        method: 'POST',
        headers: { 'User-Agent': UA, 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `data=${encodeURIComponent(query)}`,
      });
      if (!overpassRes.ok) return res.status(500).json({ error: 'Overpass API error' });
      ...
      return res.json({ places, source: 'openstreetmap', suggested_category_id: suggestedCategoryId });
    } catch (err: unknown) {
      console.error('Overpass nearby error:', err);
      return res.status(500).json({ error: 'OpenStreetMap nearby search error' });
    }
  }
  ...
  try {
    ...
    res.json({ places, source: 'google', suggested_category_id: suggestedCategoryId });
  } catch (err: unknown) {
    console.error('Nearby search error:', err);
    res.status(500).json({ error: 'Google Places nearby search error' });
  }
});
